    ...
    var body: some View {
        TimelineView(.animation) { timeline in
            ...
            Canvas { context, size in
                ...
                let base = Gradient(colors: [Color.blue, Color.green, Color.indigo])
                ...
                blob(CGPoint(x: x1, y: y1), min(size.width, size.height) * 0.35, Color.white)
                blob(CGPoint(x: x2, y: y2), min(size.width, size.height) * 0.30, Color.white)
