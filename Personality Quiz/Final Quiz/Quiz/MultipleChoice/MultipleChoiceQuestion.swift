...
struct MultipleChoiceQuestion: View {
    @State private var viewModel: MultipleChoiceQuestionViewModel

    init(question: MultipleChoice, results: [House] = []) {
        _viewModel = State(initialValue: MultipleChoiceQuestionViewModel(question: question, results: results))
    }
